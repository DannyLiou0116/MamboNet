...
# JLLIU 
# 下述同 pykitti 的 get_velo 函式，scan reshape 後即 shape = (point_sum, 4_feature) 
# scan 的直行特徵為x, y, z, i
# 最後的 self.set_points(points, remissions) 與其接下來的 do_range_projection 函式，我們主要著重在他們轉換至 64*2048 的方式
# 之後要轉回來，在 save_bins 函式使用

        # if all goes well, open pointcloud
        scan = np.fromfile(filename, dtype=np.float32)
        scan = scan.reshape((-1, 4))

        # scan.shape = (point_sum, 4_feature)
...
# JLLIU 
# DA設定是False，先註解掉看看（之後轉回x,y,z,i比較方便）
...
#JLLIU: 
# proj_xyz.shape:  (64, 2048, 3) 可知各個 H,W 對應的 x,y,z 座標都已經存在裡面了！
# intensity 則是看 proj_remission[proj_y, proj_x]
